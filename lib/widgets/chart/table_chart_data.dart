...
class TableChartData extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (dataProvider.selectedCategory == AnalysisCategory.countryTech ||
      ...
      dataTable = _buildDataTableNormal(dataProvider, years, techCode, countries, companies, academics);
    } else {
  ...
  DataTable _buildDataTableNormal(AnalysisDataProvider dataProvider, RangeValues years, String? techCode, List<String> countries, List<String> companies, List<String> academics) {
    return DataTable(
      headingRowColor: WidgetStateProperty.all(const Color.fromARGB(255, 16, 72, 98)),
      columns: [
        const DataColumn(
          label: Text('Rank', style: TextStyle(fontSize: 12, color: Colors.white)),
        ),
        const DataColumn(
          label: Text('국가', style: TextStyle(fontSize: 12, color: Colors.white)),
        ),
        if (dataProvider.selectedCategory == AnalysisCategory.companyTech)
          const DataColumn(
            label: Text('기업', style: TextStyle(fontSize: 12, color: Colors.white)),
          ),
        if (dataProvider.selectedCategory == AnalysisCategory.academicTech)
          const DataColumn(
            label: Text('대학', style: TextStyle(fontSize: 12, color: Colors.white)),
          ),
        ...List.generate(
            (years.end - years.start).toInt() + 1, (index) => DataColumn(label: Text((years.start.toInt() + index).toString(), style: const TextStyle(fontSize: 12, color: Colors.white)))),
      ],
      rows: List<DataRow>.generate(
        ...
        (index) {
          final tableData = dataProvider.getChartData(
              ...
              techCode: techCode,
              country: dataProvider.selectedCategory == AnalysisCategory.countryTech ? countries[index] : null,
              targetName: dataProvider.selectedCategory == AnalysisCategory.companyTech
                  ? companies[index]
                  : dataProvider.selectedCategory == AnalysisCategory.academicTech
                      ? academics[index]
                      : null);
          var countryCode = dataProvider.selectedCategory == AnalysisCategory.countryTech
              ? countries[index].replaceAll(RegExp(r'[\[\]]'), '')
              : dataProvider.selectedCategory == AnalysisCategory.companyTech
                  ? dataProvider.searchCountryCode(companies[index]).replaceAll(RegExp(r'[\[\]]'), '')
                  : dataProvider.searchCountryCode(academics[index]).replaceAll(RegExp(r'[\[\]]'), '');
          return DataRow(
            cells: [
              ...
              ...List.generate(
                ...
                (yearIndex) => DataCell(
                  Text(tableData[(years.start.toInt() + yearIndex)]?.toStringAsFixed(3) ?? '0.000'),
                ),
              ),
            ],
          );
  ...
  DataTable _buildDataTableTechCompetition(AnalysisDataProvider dataProvider, Map<String, Map<String, double>> data, List<String> dataCodes) {
    return DataTable(
      headingRowColor: WidgetStateProperty.all(const Color.fromARGB(255, 16, 72, 98)),
      columns: [
        const DataColumn(
        ...
        ...dataCodes.map(
          (code) => DataColumn(
            label: SizedBox(
              width: 80,
              child: Text(code, style: const TextStyle(fontSize: 12, color: Colors.white)),
            ),
          ),
        ),
      ],
        ...
        (index) {
          return DataRow(
            cells: [
              ...
              ...dataCodes.map(
                (code) => DataCell(
                  Text(data[data.keys.elementAt(index)]?[code]?.toStringAsFixed(3) ?? '0.000'),
                ),
              ),
            ],
          );
